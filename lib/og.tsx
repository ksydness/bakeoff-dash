...
export const OG_BRAND = { emoji: '🧁', name: 'Fantasy Bake Off', tagline: 'Friends League · Dashboard & Draft Hub' };
  ...
  return new ImageResponse(
    (
      <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#0c0a09', color: '#fafaf9', fontFamily: 'sans-serif', position: 'relative' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'radial-gradient(circle at 50% 38%, rgba(249,168,212,0.20), rgba(12,10,9,0) 62%)' }} />
        <div style={{ fontSize: 118, lineHeight: 1, display: 'flex' }}>{OG_BRAND.emoji}</div>
        <div style={{ marginTop: 22, fontSize: heading.length > 18 ? 68 : 86, fontWeight: 700, letterSpacing: -2, background: 'linear-gradient(90deg,#f9a8d4,#93c5fd)', backgroundClip: 'text', color: 'transparent', display: 'flex' }}>{heading}</div>
        ...
        <div style={{ position: 'absolute', bottom: 34, left: 0, right: 0, display: 'flex', justifyContent: 'center', fontSize: 24, color: '#78716c' }}>bakeoff-dash.vercel.app</div>
      </div>
    ),
    { ...OG_SIZE },
  );
