...
// Decimal-safe (Bake Off scoring produces half-points, e.g. 10.5 / 213.5 —
// Survivor's integer-only parseInt version would read 10.5 as 105).
const num = (v: string) => {
...
export function parseLeaderboard(rows: string[][], numWeeks: number) {
  ...
  const splitNamePts = (s: string) => {
    // decimal-aware: "Lenny (25.5)" as well as "Toby (15)"
    const m = (s || '').match(/^(.*?)\s*\((-?\d+(?:\.\d+)?)\)\s*$/);
    return m ? { name: m[1].trim(), pts: parseFloat(m[2]) } : { name: (s || '').trim(), pts: undefined };
  };
