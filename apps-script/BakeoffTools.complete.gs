/**
 * Bakeoff Tools — complete script (scoring + season automation)
 * ==============================================================
...
const HUB_SPREADSHEET_ID = 'PASTE_YOUR_BAKEOFF_HUB_SPREADSHEET_ID_HERE'; // the long ID in this workbook's URL
...
const DEFAULT_WEEKS      = 9;   // GBBO: Week 2..Week 10 (scoring starts after the draft)
// ---------------------------------------------
...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu('Bakeoff Tools')
    .addItem('Update Scores', 'updateBakeoffScores')
...
function startNewSeason() {
  ...
  const sResp = ui.prompt('Start New Season', 'Season number (e.g. 2):', ui.ButtonSet.OK_CANCEL);
  ...
  const wResp = ui.prompt('Start New Season', 'Number of weeks (e.g. 9):', ui.ButtonSet.OK_CANCEL);
  ...
  if (have.indexOf(season) === -1) {
    reg.appendRow([season, 'Season ' + season, 'active', epUrl, coUrl, lbUrl, scUrl, weeks, 'Lenny|Daegan']);
  } else {
...
function finalizeSeason() {
  ...
  const resp = ui.prompt('Finalize Season', 'Which season number is finishing? (e.g. 2):', ui.ButtonSet.OK_CANCEL);
...
function resetSeasonData_(ss) {
  ...
  if (ep && ep.getLastRow() > 1 && ep.getLastColumn() > 1) {
    // clear only the week columns (B..J); column K holds the Total formulas
    ep.getRange(2, 2, ep.getLastRow() - 1, 9).clearContent();
  }
  const lb = ss.getSheetByName('Leaderboard');
...
function retroactiveUpdate() {
  ...
  var episodesData = episodesSheet.getRange(2, 1, episodesSheet.getLastRow() - 1, 10).getValues();
  var contestants = episodesData.map(row => row[0]); // Contestant names from column A
  var pointsData = episodesData.map(row => row.slice(1, 10)); // Points from columns B to J
  ...
  for (var col = 2; col <= 10; col++) { // Columns B to J
...
function updateBakeoffScores() {
  ...
  // Find the next available week column (B to J, columns 2 to 10 for Week 2 to Week 10)
  ...
  if (nextWeekColumn == -1) {
    SpreadsheetApp.getUi().alert('All week columns (Week 2 to Week 10) are filled. Cannot add more weeks.');
    return;
  }
