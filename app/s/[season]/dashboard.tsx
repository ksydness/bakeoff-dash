...
const TEAM_COLORS: Record<string, string> = {
  'Lenny': '#f9a8d4',   // tent pink
  'Daegan': '#93c5fd',  // tent blue
};
...
const TABS: [Tab, string][] = [
  ['leaderboard', 'Leaderboard'], ['teams', 'Teams'], ['contestants', 'Bakers'],
    ...
    <>
      ...
      <div id="app">
        ...
        <div className="header">
          ...
          <h1>{props.title ?? 'Fantasy Bake Off'}</h1>
    ...
    <>
      ...
      {d.highlights.length > 0 && (
        <div className="panel">
          ...
          <table><thead><tr><th>Wk</th><th>Top Baker</th><th className="num">Pts</th><th>Top Team</th><th className="num">Pts</th></tr></thead>
    ...
    <>
      <div className="panel"><h2>Season Records</h2>
        <div className="stats">
          ...
          <Stat k="Top Baker" v={top[0]?.name} dd={`${top[0]?.total} pts · ${top[0]?.team}`} />
          <Stat k="Biggest Single Week" v={String(bestWk.v)} dd={`${bestWk.p} · Week ${bestWk.wk}`} />
          <Stat k="Margin (1st–last)" v={String(spread)} dd="points" />
          <Stat k="Total Points" v={String(allPts)} dd={`${d.contestants.length} bakers`} />
        </div>
      </div>
      <div className="panel"><h2>Top 5 Bakers</h2>
        <table><thead><tr><th>#</th><th>Baker</th><th>Team</th><th className="num">Pts</th></tr></thead>
      ...
      {negs.length > 0 && (
        <div className="panel"><h2>Negative Weeks</h2>
          <table><thead><tr><th>Baker</th><th className="num">Week</th><th className="num">Pts</th></tr></thead>
...
.header h1{font-size:28px;font-weight:800;letter-spacing:-.02em;margin-top:6px;background:linear-gradient(90deg,#f9a8d4,#93c5fd);-webkit-background-clip:text;background-clip:text;color:transparent}
...
.tab.active{color:#1c1917;background:linear-gradient(90deg,#f9a8d4,#93c5fd)}
