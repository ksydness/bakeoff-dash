...
export default function DraftRoom({ season }: { season: number }) {
  ...
  const lsKey = `bakeoff-draft-s${season}`;
      ...
      {phase === 'setup' && (
        <div className="panel center">
          ...
          <p className="dim small">
            {cast.length} bakers · {teams.length} teams · {picksPerTeam * teams.length} of {cast.length} drafted
...
.header h1{font-size:26px;font-weight:800;letter-spacing:-.02em;margin-top:6px;background:linear-gradient(90deg,#f9a8d4,#93c5fd);-webkit-background-clip:text;background-clip:text;color:transparent}
...
.btn.primary{background:linear-gradient(90deg,#f9a8d4,#93c5fd);color:#1c1917;border:none}
...
.pick:hover .draftbtn{color:#f9a8d4}
...
.board .cell.now{background:#2a2520;outline:2px solid #f9a8d4;outline-offset:-2px}
